...
def job_config = [
        job: [
                name: "asddasdas_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asddasdas_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asddasdas_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asddasdas.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asddasdas", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asddasdas_Pipeline_develop");

job.with {

    authenticationToken('utC8popIIVLrKDrYXwFy8T4uxMxWceYc')
